class AggregateTransaction < ApplicationRecord
  def self.total_summary
    records = AggregateTransaction.where('qty != 0')
    ...
    date = Date.yesterday
    infos = AggregateTxSnapshotInfo.includes(:aggregate_tx_snapshot_records).where("event_date <= ?", date)
    {
      ...
      max_profit_date: $redis.get("user_#{date.to_s}_aggregate_spots_max_profit_date"),
      max_loss: infos.max_loss,
      max_loss_date: $redis.get("user_#{date.to_s}_aggregate_spots_max_loss_date"),
      max_revenue: infos.max_revenue,
      max_revenue_date: $redis.get("user_#{date.to_s}_aggregate_spots_max_revenue_date"),
      min_revenue: infos.min_revenue,
      min_revenue_date: $redis.get("user_#{date.to_s}_aggregate_spots_min_revenue_date"),
      max_roi: infos.max_roi,
      max_roi_date: $redis.get("user_#{date.to_s}_aggregate_spots_max_roi_date"),
      min_roi: infos.min_roi,
      min_roi_date: $redis.get("user_#{date.to_s}_aggregate_spots_min_roi_date"),
      max_profit_roi: infos.max_profit_roi,
      max_profit_roi_date: $redis.get("user_#{date.to_s}_aggregate_spots_max_profit_roi_date"),
      max_loss_roi: infos.max_loss_roi,
      max_loss_roi_date: $redis.get("user_#{date.to_s}_aggregate_spots_max_loss_roi_date")
    }
  end
