class AggregateTxSnapshotInfo < ApplicationRecord
  has_many :aggregate_tx_snapshot_records, dependent: :destroy

  def total_profit
    aggregate_tx_snapshot_records.profit.sum(&:revenue)
  end

  def total_loss
    aggregate_tx_snapshot_records.loss.sum(&:revenue)
  end

  def total_revenue
    aggregate_tx_snapshot_records.sum(&:revenue)
  end

  def total_cost
    aggregate_tx_snapshot_records.sum(&:amount)
  end
  ...
  def self.max_profit(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_max_profit"
    total_profit = $redis.get(redis_key).to_f
    if total_profit == 0
      infos = AggregateTxSnapshotInfo.joins(:aggregate_tx_snapshot_records)
  ...
  def self.max_loss(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_max_loss"
  ...
  def self.max_revenue(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_max_revenue"
  ...
  def self.min_revenue(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_min_revenue"
    total_revenue = $redis.get(redis_key).to_f
    if total_revenue == 0
      infos = AggregateTxSnapshotInfo.joins(:aggregate_tx_snapshot_records)
  ...
  def self.max_roi(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_max_roi"
  ...
  def self.min_roi(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_min_roi"
    total_roi = $redis.get(redis_key).to_f
    if total_roi == 0
      infos = AggregateTxSnapshotInfo.joins(:aggregate_tx_snapshot_records)
  ...
  def self.max_profit_roi(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_max_profit_roi"
    total_roi = $redis.get(redis_key).to_f
    if total_roi == 0
      infos = AggregateTxSnapshotInfo.joins(:aggregate_tx_snapshot_records)
  ...
  def self.max_loss_roi(date: Date.yesterday)
    redis_key = "user_#{date.to_s}_aggregate_spots_max_loss_roi"
    total_roi = $redis.get(redis_key).to_f
    if total_roi == 0
      infos = AggregateTxSnapshotInfo.joins(:aggregate_tx_snapshot_records)
