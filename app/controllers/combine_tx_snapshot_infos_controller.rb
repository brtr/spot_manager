class CombineTxSnapshotInfosController < ApplicationController
  def index
    @page_index = 6
    infos = CombineTxSnapshotInfo.pluck(:id, :event_date)
    @results = split_event_dates(infos)
  end

  def show
    @page_index = 6
    @info = CombineTxSnapshotInfo.find_by(id: params[:id])
    sort = params[:sort].presence || "revenue"
    sort_type = params[:sort_type].presence || "desc"
    records = @info.combine_tx_snapshot_records
